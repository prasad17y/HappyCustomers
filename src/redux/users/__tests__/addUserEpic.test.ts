import {StateObservable} from 'redux-observable';
import {of, Subject} from 'rxjs';
import {toArray} from 'rxjs/operators';
import {addUserEpic} from '../epics';
import {
  addUserRequest,
  submitUserSuccess,
  submitUserFailure,
  setMutating,
} from '../actions';
import {showToast} from '../../notifications/actions';
import * as dbActions from '../../../db/actions';
import {RootState} from '../../root';
import {Role} from '../../../types/types';

// Mock the db/actions module
jest.mock('../../../db/actions');

describe('addUserEpic', () => {
  let state$: StateObservable<RootState>;

  const createMockState = (isMutating: boolean) => {
    const mockState: Partial<RootState> = {
      users: {isMutating} as any,
    };
    return new StateObservable(new Subject(), mockState as RootState);
  };

  beforeEach(() => {
    (dbActions.addUser as jest.Mock).mockClear();
  });

  it('should handle successful user addition', done => {
    (dbActions.addUser as jest.Mock).mockResolvedValue(undefined);
    const mockUser = {
      firstName: 'New',
      lastName: 'User',
      role: Role.MANAGER,
      email: null,
    };
    const action$ = of(addUserRequest(mockUser));
    state$ = createMockState(false);

    const expectedActions = [
      setMutating(true),
      submitUserSuccess(),
      showToast({message: 'User created successfully!', type: 'success'}),
    ];

    addUserEpic(action$ as any, state$, undefined)
      .pipe(toArray())
      .subscribe(outputActions => {
        expect(outputActions).toEqual(expectedActions);
        done();
      });
  });

  it('should handle failed user addition', done => {
    (dbActions.addUser as jest.Mock).mockRejectedValue(new Error('DB Error'));
    const mockUser = {
      firstName: 'New',
      lastName: 'User',
      role: Role.MANAGER,
      email: null,
    };
    const action$ = of(addUserRequest(mockUser));
    state$ = createMockState(false);

    const expectedActions = [
      setMutating(true),
      submitUserFailure(),
      showToast({message: 'Could not create user.', type: 'error'}),
    ];

    addUserEpic(action$ as any, state$, undefined)
      .pipe(toArray())
      .subscribe(outputActions => {
        expect(outputActions).toEqual(expectedActions);
        done();
      });
  });

  it('should dispatch a busy toast if a mutation is already in progress', done => {
    const mockUser = {
      firstName: 'New',
      lastName: 'User',
      role: Role.MANAGER,
      email: null,
    };
    const action$ = of(addUserRequest(mockUser));
    state$ = createMockState(true); // Set isMutating to true

    const expectedActions = [
      showToast({
        message: 'Operation in progress. Please try again later.',
        type: 'error',
      }),
    ];

    addUserEpic(action$ as any, state$, undefined)
      .pipe(toArray())
      .subscribe(outputActions => {
        expect(outputActions).toEqual(expectedActions);
        // Ensure the database action was NOT called
        expect(dbActions.addUser).not.toHaveBeenCalled();
        done();
      });
  });
});
